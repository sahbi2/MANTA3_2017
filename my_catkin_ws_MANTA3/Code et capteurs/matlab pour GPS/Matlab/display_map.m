...
%%%OUVERTURE FICHIER
%id=fopen('results-exp-2.txt');
id=fopen('results.txt');
textscan(id,'T');
[Ttemp]=cell2mat(textscan(id,'%f'));
%le repere utilis� sera centre sur la position GPS de depart
textscan(id,'GPS');
...
while ~feof(id)
    ...
    textscan(id,'OR');
    [ORtemp]=cell2mat(textscan(id,'%f'));
    %position GPS par rapport � la position de depart en utilisant distance
